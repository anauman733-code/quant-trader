//+------------------------------------------------------------------+
//| XAU_HyperScalper_V6_1.mq5                                       |
//| Rolling tick micro-range breakout + burst/velocity engine        |
//| Demo-first research EA. No martingale, grid or averaging.        |
//+------------------------------------------------------------------+
#property strict
#property version   "6.10"
#property description "Demo-first XAUUSD rolling micro-range breakout scalper confirmed by bid-tick burst, velocity and short momentum."
...
input long   InpMagicNumber                 = 26090861;
...
input int    InpPendingLifeMs               = 800;
...
{
   if(InpVerboseLogging) Print("[XAU-HS61] ",s);
}
...
string MakeDayKey()
{
   ...
   return StringFormat("XAUHS61_%I64d_%I64d_%04d%02d%02d",login,InpMagicNumber,dt.year,dt.mon,dt.day);
}
...
bool GetMicroRange(double &hi,double &lo,double &widthPts)
{
   ...
   // Exclude the current tick (offset 0). Current tick is what breaks the channel.
   ...
   if(widthPts<InpMinMicroRangePoints || widthPts>InpMaxMicroRangePoints) return false;
   return true;
}
...
bool GetOurPending(ulong &ticket,ENUM_ORDER_TYPE &type,double &price,long &setupMs)
{
   ...
   for(int i=OrdersTotal()-1;i>=0;--i)
   {
      ...
      ticket=t; type=ot; price=OrderGetDouble(ORDER_PRICE_OPEN); setupMs=(long)OrderGetInteger(ORDER_TIME_SETUP_MSC);
      return true;
   }
   return false;
}
...
bool PlaceContinuationEntry(const int dir,const MqlTick &tick)
{
   ...
   double sl=NormalizeDouble((dir>0?entry-stopPts*_Point:entry+stopPts*_Point),digits);
   double tp=NormalizeDouble((dir>0?entry+tpPts*_Point:entry-tpPts*_Point),digits);
   ...
   {
      if(dir>0) ok=trade.BuyStop(InpFixedLots,entry,_Symbol,sl,tp,ORDER_TIME_GTC,0,"XAU-HS61 BSTOP");
      else      ok=trade.SellStop(InpFixedLots,entry,_Symbol,sl,tp,ORDER_TIME_GTC,0,"XAU-HS61 SSTOP");
   }
   else
   {
      trade.SetDeviationInPoints(InpMaxFillSlippagePoints);
      if(dir>0) ok=trade.Buy(InpFixedLots,_Symbol,0.0,sl,tp,"XAU-HS61 BUY");
      else      ok=trade.Sell(InpFixedLots,_Symbol,0.0,sl,tp,"XAU-HS61 SELL");
   }
   ...
   g_expectedDir=dir;
   Log(StringFormat("ENTRY %s %s trigger=%.2f sl=%.2f tp=%.2f spread=%.1f",DirText(dir),(InpUseContinuationStopEntry?"STOP":"MARKET"),entry,sl,tp,SpreadPts()));
   return true;
}
...
void Heartbeat(const MqlTick &tick,const double hi,const double lo,const double width,const bool rangeOK)
{
   ...
   Log(StringFormat("HB state=%s arm=%s microRange=%.1f valid=%s hi=%.2f lo=%.2f spread=%.1f losses=%d gate=%s",
       (CountOurPositions()>0?"POSITION":(pend?"PENDING":"FLAT")),DirText(g_armDir),width,(rangeOK?"YES":"NO"),hi,lo,SpreadPts(),g_consecutiveLosses,gate));
}
...
int OnInit()
{
   ...
   if(!IsDemoAllowed()) return INIT_FAILED;
   if(!IsFullTradeSymbol()) return INIT_FAILED;
   trade.SetExpertMagicNumber(InpMagicNumber);
   trade.SetTypeFillingBySymbol(_Symbol);
   RefreshDayEquity();
   RebuildLossStreak();
   Log(StringFormat("Initialized V6.1 on %s. ROLLING MICRO-RANGE engine lots=%.2f maxTrades/hr=%d",_Symbol,InpFixedLots,InpMaxTradesPerHour));
   Log("Signal path: recent-tick channel breakout -> 2-tick burst -> velocity -> short momentum -> continuation STOP.");
...
void OnTick()
{
   ...
   if(g_armDir==0)
   {
      if(tick.bid>=hi+InpBreakoutBufferPoints*_Point)
      {
         ...
         Log(StringFormat("ARM BUY microHi=%.2f width=%.1f",hi,width));
      }
      else if(tick.bid<=lo-InpBreakoutBufferPoints*_Point)
      {
         g_armDir=-1; g_breakoutLevel=lo; g_armSerial=g_tickSerial;
         Log(StringFormat("ARM SELL microLo=%.2f width=%.1f",lo,width));
      }
   }
   ...
   if(g_armDir!=0)
   {
      ...
      else
      {
         ...
         if(b && m)
         {
            Log(StringFormat("CONFIRM %s burst=%.1f vel=%.1f net=%.1f bias=%.2f pressure=%.0f chase=%.1f",DirText(g_armDir),burst,vel,net,bias,pressure,chase));
            int dir=g_armDir;
            ResetArm("consumed");
   ...
   if(e==DEAL_ENTRY_IN || e==DEAL_ENTRY_INOUT)
   {
      ...
      if(g_expectedEntry>0.0 && dir==g_expectedDir)
      {
         ...
         if(InpMaxFillSlippagePoints>0 && slip>InpMaxFillSlippagePoints)
         {
            ...
            Log(StringFormat("BAD FILL %.1fpts > %dpts",slip,InpMaxFillSlippagePoints));
         }
      }
