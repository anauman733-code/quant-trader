//+------------------------------------------------------------------+
//| XAU_HyperScalper_V6_2.mq5                                      |
//| V6 fast-quality breakout + micro-flow bias + failure exit       |
//| Demo-first research EA. No martingale, grid or averaging.       |
//+------------------------------------------------------------------+
#property strict
#property version   "6.20"
#property description "Demo-first XAUUSD M1 range-burst scalper with micro-flow direction guard and fast failed-breakout exit."
...
input long   InpMagicNumber                 = 26090864;
...
input group "Micro Flow Guard"
input bool   InpUseMicroFlowGuard           = true;
input int    InpFlowWindowTicks             = 32;
input double InpMinFlowNetPoints            = 10.0;
input double InpMinFlowBias                 = 0.56;
input double InpMinFlowVelocityPointsPerSec = 2.0;

input group "Execution"
input bool   InpUseContinuationStopEntry    = true;
input int    InpEntryStopOffsetPoints       = 2;
input int    InpPendingLifeMs               = 1000;
input int    InpMaxSpreadPoints             = 18;
input double InpMinTargetToSpreadRatio      = 5.0;
input int    InpMaxFillSlippagePoints       = 12;
input int    InpCloseSlippagePoints         = 20;

input group "Stop / Target"
input int    InpStopLossPoints              = 120;
input int    InpTakeProfitPoints            = 180;
input int    InpBreakEvenTriggerPoints      = 65;
input int    InpBreakEvenLockPoints         = 5;
input int    InpTrailStartPoints            = 100;
input int    InpTrailDistancePoints         = 40;
input int    InpMaxHoldSeconds              = 40;

input group "Fast Failed-Breakout Exit"
input bool   InpUseFailedBreakoutExit       = true;
input int    InpFailedBreakoutMinHoldMs     = 600;
input int    InpFailedBreakoutWindowMs      = 6000;
input int    InpFailedBreakoutReclaimPoints = 8;
...
double g_pendingBreakoutLevel = 0.0;
double g_activeBreakoutLevel = 0.0;
...
{
   if(InpVerboseLogging) Print("[XAU-HS62] ", s);
}
...
string MakeDayKey()
{
   ...
   return StringFormat("XAUHS62_%I64d_%I64d_%04d%02d%02d",login,InpMagicNumber,dt.year,dt.mon,dt.day);
}
...
bool MicroFlowState(int &dir,double &netPts,double &bias,double &velocity)
{
   dir=0; netPts=0.0; bias=0.0; velocity=0.0;
   int n=InpFlowWindowTicks;
   if(!InpUseMicroFlowGuard) return true;
   if(n<4 || g_count<n) return false;

   double newest=0.0,oldest=0.0; long newestMs=0,oldestMs=0;
   if(!TickAt(0,newest,newestMs) || !TickAt(n-1,oldest,oldestMs)) return false;
   netPts=(newest-oldest)/_Point;
   ...
   long dt=newestMs-oldestMs;
   if(dt<=0) return false;
   velocity=MathAbs(netPts)/((double)dt/1000.0);

   if(netPts>=InpMinFlowNetPoints)
   {
      bias=(double)up/(double)meaningful;
      if(bias>=InpMinFlowBias && velocity>=InpMinFlowVelocityPointsPerSec) dir=1;
   }
   else if(netPts<=-InpMinFlowNetPoints)
   {
      bias=(double)down/(double)meaningful;
      if(bias>=InpMinFlowBias && velocity>=InpMinFlowVelocityPointsPerSec) dir=-1;
   }
   return (dir!=0);
}
...
void DeleteAllPending(const string why)
{
   ...
   for(int i=OrdersTotal()-1;i>=0;--i)
   {
      ...
      {
         deleted=true;
         Log(StringFormat("DELETE pending #%I64u reason=%s",t,why));
      }
   }
   if(deleted) g_pendingBreakoutLevel=0.0;
}
...
bool PlaceContinuationEntry(const int dir,const MqlTick &tick,const double breakoutLevel)
{
   ...
   {
      if(dir>0) ok=trade.BuyStop(InpFixedLots,entry,_Symbol,sl,tp,ORDER_TIME_GTC,0,"XAU-HS62 BSTOP");
      else      ok=trade.SellStop(InpFixedLots,entry,_Symbol,sl,tp,ORDER_TIME_GTC,0,"XAU-HS62 SSTOP");
   }
   else
   {
      trade.SetDeviationInPoints(InpMaxFillSlippagePoints);
      if(dir>0) ok=trade.Buy(InpFixedLots,_Symbol,0.0,sl,tp,"XAU-HS62 BUY");
      else      ok=trade.Sell(InpFixedLots,_Symbol,0.0,sl,tp,"XAU-HS62 SELL");
   }
   ...
   g_expectedDir=dir;
   g_pendingBreakoutLevel=breakoutLevel;
   Log(StringFormat("ENTRY %s %s lots=%.2f trigger=%.2f breakout=%.2f sl=%.2f tp=%.2f spread=%.1f",
                    DirText(dir),(InpUseContinuationStopEntry?"STOP":"MARKET"),InpFixedLots,entry,breakoutLevel,sl,tp,SpreadPts()));
   return true;
}
...
void ManagePosition(const MqlTick &tick)
{
   ...
   if(InpUseFailedBreakoutExit && g_activeBreakoutLevel>0.0 &&
      held>=InpFailedBreakoutMinHoldMs && held<=InpFailedBreakoutWindowMs)
   {
      bool failed=false;
      if(type==POSITION_TYPE_BUY)
         failed=(tick.bid <= g_activeBreakoutLevel-InpFailedBreakoutReclaimPoints*_Point);
      else
         failed=(tick.ask >= g_activeBreakoutLevel+InpFailedBreakoutReclaimPoints*_Point);

      if(failed)
      {
         Log(StringFormat("FAILED BREAKOUT %s held=%I64dms p=%.1f level=%.2f",(type==POSITION_TYPE_BUY?"BUY":"SELL"),held,p,g_activeBreakoutLevel));
         ClosePosition(ticket,"failed-breakout-fast-exit");
         return;
      }
   }
...
void Heartbeat(const MqlTick &tick,const double hi,const double lo,const double width)
{
   ...
   int flowDir=0; double flowNet=0.0,flowBias=0.0,flowVel=0.0;
   MicroFlowState(flowDir,flowNet,flowBias,flowVel);

   Log(StringFormat("HB state=%s arm=%s range=%.1f spread=%.1f flow=%s net=%.1f bias=%.2f vel=%.1f losses=%d gate=%s",
                    (CountOurPositions()>0?"POSITION":(pend?"PENDING":"FLAT")),DirText(g_armDir),width,SpreadPts(),DirText(flowDir),flowNet,flowBias,flowVel,g_consecutiveLosses,gate));
}
...
int OnInit()
{
   ...
   if(InpRangeLookbackBars<1 || InpBurstTicks<2 || InpMomentumWindowTicks<3)
      return INIT_PARAMETERS_INCORRECT;
   if(InpUseMicroFlowGuard && (InpFlowWindowTicks<4 || InpFlowWindowTicks>=TICK_BUF))
      return INIT_PARAMETERS_INCORRECT;
   if(!IsDemoAllowed())
   {
      Print("[XAU-HS62] Demo-only guard blocked real account initialization.");
      return INIT_FAILED;
   }
   if(!IsFullTradeSymbol())
   {
      Print("[XAU-HS62] Symbol not FULL ACCESS. Use your broker's tradable gold symbol, e.g. XAUUSD.a.");
      return INIT_FAILED;
   }
   ...
   {
      Print("[XAU-HS62] Indicator handle error=",GetLastError());
      return INIT_FAILED;
   }
   ...
   RefreshDayEquity();
   RebuildLossStreak();
   Log(StringFormat("Initialized V6.2 on %s. FAST-QUALITY + MICRO-FLOW engine lots=%.2f maxTrades/hr=%d",_Symbol,InpFixedLots,InpMaxTradesPerHour));
   Log("Signal path: 2-bar M1 range -> breakout -> burst/velocity -> short momentum -> 32-tick flow agreement -> continuation STOP.");
   Log("Exit upgrade: if breakout is reclaimed quickly, cut the trade before the full hard stop.");
...
void OnTick()
{
   ...
   if(g_armDir!=0)
   {
      ...
      else
      {
         ...
         int flowDir=0; double flowNet=0.0,flowBias=0.0,flowVel=0.0;
         bool flowOK=MicroFlowState(flowDir,flowNet,flowBias,flowVel);

         if(b && m && f)
         {
            if(InpUseMicroFlowGuard && (!flowOK || flowDir!=g_armDir))
            {
               Log(StringFormat("REJECT FLOW signal=%s flow=%s net=%.1f bias=%.2f vel=%.1f",
                                DirText(g_armDir),DirText(flowDir),flowNet,flowBias,flowVel));
               ResetArm("flow-disagree");
            }
            else
            {
               Log(StringFormat("CONFIRM %s burst=%.1f vel=%.1f net=%.1f bias=%.2f pressure=%.0f flowNet=%.1f flowBias=%.2f flowVel=%.1f chase=%.1f",
                                DirText(g_armDir),burst,vel,net,bias,pressure,flowNet,flowBias,flowVel,chase));
               int dir=g_armDir;
               double level=g_breakoutLevel;
               ResetArm("consumed");
               PlaceContinuationEntry(dir,tick,level);
            }
         }
      }
   }
...
void OnTradeTransaction(const MqlTradeTransaction &trans,const MqlTradeRequest &request,const MqlTradeResult &result)
{
   ...
   if(e==DEAL_ENTRY_IN || e==DEAL_ENTRY_INOUT)
   {
      ...
      g_activeBreakoutLevel=g_pendingBreakoutLevel;
      g_pendingBreakoutLevel=0.0;

      if(g_expectedEntry>0.0 && dir==g_expectedDir)
      {
         double slip=(dir>0?(fill-g_expectedEntry):(g_expectedEntry-fill))/_Point;
         Log(StringFormat("FILL %s price=%.2f expected=%.2f breakout=%.2f slippage=%.1fpts",DirText(dir),fill,g_expectedEntry,g_activeBreakoutLevel,slip));
