...
def make_features(bids, asks, times_ms, point):
    ...
    a15=_bias(bids[-15:]); a8=_bias(bids[-8:]); a30b=_bias(bids[-30:]); a50b=_bias(bids[-50:])
    x=bids[-50:]; mean=float(x.mean()); sd=float(x.std()); z50=(x[-1]-mean)/sd if sd>0 else 0.0
    last_delta=(bids[-1]-bids[-2])/point
    prev_delta=(bids[-2]-bids[-3])/point
    accel=last_delta-prev_delta
    sec=(int(times_ms[-1]//1000)%86400)
    ang=2.0*math.pi*sec/86400.0
